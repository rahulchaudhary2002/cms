        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header d-flex-space-between align-center">
                    <h1 class="card-title">Examination Records List</h1>
                </div>
                <div class="card-body expand">
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            @forelse($result->examinationRecords as $key => $examinationRecord)
                            <tr>
                                <td>{{ ++$key }}</td>
                                <td>{{ $examinationRecord->examinationStage->semester->name }}</td>
                                <td>{{ $examinationRecord->examinationStage->name }}</td>
                                <td>{{ $examinationRecord->gpa }}</td>
                                <td>
                                    <a class="text-success" title="View Submission" href="{{ route('examination.record.show', [$examinationRecord->examinationStage->key, auth()->user()->key]) }}"><i class="fa fa-eye"></i></a>
                                </td>
                            </tr>
                            @empty
                            <tr>
                                <td colspan="5">No examination records found at this moment.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
            ...
            <div class="card">
                <div class="card-header d-flex-space-between align-center">
                    ...
                    <div class="card-setting d-flex gap-2">
                        ...
                        <a class="btn btn-sm btn-outline-warning text-center btn-rounded" href="{{ route('examination.record.import') }}"><span class="fa fa-plus"></span> import excel</a>
                        @endif
                    </div>
                </div>
                <div class="card-body expand">
        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header d-flex-space-between align-center">
                    ...
                    @if(auth()->user()->can('examination-record.create'))
                    <div class="card-setting d-flex gap-2">
                        <a class="btn btn-sm btn-outline-warning" href="{{ route('examination.record.template.export') . '?academic_year=' . request()->get('academic_year') . '&program=' . request()->get('program') . '&semester=' . request()->get('semester') . '&session=' . request()->get('session') . '&examination_stage=' . request()->get('examination_stage') }}"><span class="fa fa-download"></span> Download Template</a>
                    </div>
                    @endif
                </div>
                <div class="card-body">
                    <table class="table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    @if(examinationRecord($student, request()->get('examination_stage')))
                                    ...
                                    @endif
                                    @if(auth()->user()->can('examination-record.update') && examinationRecord($student, request()->get('examination_stage')))
                                    ...
                                    @endif
                                    @if(auth()->user()->can('examination-record.create') && !examinationRecord($student, request()->get('examination_stage')))
...
<script>
    ...
    function updateSessionOptions() {
        let academicYearKey = isStudent == 1 ? "{{ auth()->user()->student ? auth()->user()->student->academicYear->key : '' }}" : $('#academicYear').val();
